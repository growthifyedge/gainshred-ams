...
export default function MemberForm({
  ...
  // Auto-fill the monthly fee from plan + offer + age. Admin can still edit it.
  function recalcFee(pId: string, off: OfferCode, ag: number | null) {
    const plan = plans.find((p) => p.id === pId) ?? null;
    const r = computePackage({ plan, services: [], offer: off, age: ag });
    setFee(String(r.monthlyFee));
  }
    ...
    <form action={formAction} className="card max-w-3xl space-y-6 p-6">
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => {
              ...
              recalcFee(planId, offer, e.target.value === '' ? null : Number(e.target.value));
            }}
            className="input"
            placeholder="e.g. 30"
          />
          {pricing.isSenior && (
            <p className="mt-1 text-xs font-medium text-emerald-600">Senior Citizen Offer applied (67+)</p>
          )}
        </div>
      </div>

      {/* --- Membership duration + offer --- */}
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div>
          ...
          <select
            ...
            onChange={(e) => {
              ...
              recalcFee(e.target.value, offer, ageNum);
            }}
            className="input"
          >
            ...
            {plans.map((p) => (
              <option key={p.id} value={p.id}>
                ...
                {p.saving_amount ? ` (Save ${formatMoney(p.saving_amount)})` : ''}
              </option>
            ))}
          </select>
        </div>
        <div>
          ...
          <select
            ...
            value={offer}
            onChange={(e) => {
              const v = e.target.value as OfferCode;
              setOffer(v);
              recalcFee(planId, v, ageNum);
            }}
            className="input"
          >
            {OFFER_OPTIONS.map((o) => (
        ...
        <div>
          ...
          <p className="mt-1 text-xs text-neutral-400">Auto-filled from the package — you can override it.</p>
        </div>
        <div>
      ...
      <div className="rounded-lg bg-neutral-50 p-4 text-sm">
        ...
        <dl className="grid grid-cols-2 gap-x-6 gap-y-1">
          <Row label="Monthly fee" value={formatMoney(pricing.monthlyFee)} />
          <Row label="Advance" value={formatMoney(pricing.advance)} />
          <Row label="Services subtotal" value={formatMoney(pricing.servicesSubtotal)} />
          <Row label="Offer discount" value={formatMoney(pricing.servicesDiscount)} />
          <Row label="Services payable" value={formatMoney(pricing.servicesPayable)} />
          <Row label="Package saving" value={formatMoney(pricing.packageSaving)} />
        </dl>
        <div className="mt-2 flex items-center justify-between border-t border-neutral-200 pt-2">
          <span className="font-semibold">Suggested total (monthly + services)</span>
          <span className="text-base font-bold text-brand">{formatMoney(pricing.totalPayable)}</span>
        </div>
      </div>
